...
if (!role) return message.channel.send('**Rol bulunamadım.**');
    if (!member.roles.cache.has(role.id)) return message.channel.send(new wenzy.MessageEmbed().setDescription('Kullanıcı O Yetkiye Sahip Değil.').setColor('RANDOM'));
...
await (member.roles.remove(role.id))
 message.channel.send(new wenzy.MessageEmbed().setDescription(`${member} isimli üyenin \`${role.name}\` isimli yetkisi başarıyla aldım knk!`)  .setFooter('Bu komutu kullanan yetkili ' + message.author.tag, message.author.avatarURL).setColor('RANDOM'));
