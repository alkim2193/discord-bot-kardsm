...
exports.run = async(client, message, args, ayar, emoji) => {
  ...
  if (!message.member.hasPermission("ADMİNİSTRATOR")) {
    ...
    message.channel.send(new Discord.MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setColor(0x00bfff).setDescription(`${uye},${message.member} Seni Ses Kanalına Çekmek İstiyor, Kabul Ediyormusun? `)).then(async msj => {
      await msj.react('✅');
