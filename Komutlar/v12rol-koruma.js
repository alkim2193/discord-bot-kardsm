...
let prefix = ayarlar.prefix
  ...
  if (!args[0]) {
 message.channel.send(`**Örnek Kullanım:** ${prefix}rol-koruma aç/kapat`).then(devtr => devtr.delete({ timeout: 3000}));
  }
  if (args[0] === 'aç') {
   ...
   if (args[0] === 'kapat') {
    ...
    message.channel.send(`**Rol Koruma Başarıyla Kapatıldı!**`).then(devtr => devtr.delete({ timeout: 3000}));
  }
};
exports.conf = {
  aliases: ['rolkoruma'],
